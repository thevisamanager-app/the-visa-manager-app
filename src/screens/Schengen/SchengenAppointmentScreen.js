...
/* ---------------- GENERATE EARLIEST SLOTS ---------------- */
const generateEarliestSlots = () => {
  const today = new Date();
  today.setHours(0, 0, 0, 0); // 🔒 prevent timezone bugs

  const baseDate = new Date(today);
  baseDate.setDate(baseDate.getDate() + 5); // today + 5
...
export default function SchengenAppointmentScreen({ navigation, route }) {
  ...
  /* ================= DERIVED ================= */
  const centers = CENTER_MAP[stateName] || CENTER_MAP.default;
  const EARLY_SLOTS = useMemo(() => generateEarliestSlots(), []);
